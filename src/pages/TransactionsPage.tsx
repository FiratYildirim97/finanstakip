...
import { 
  ...
  PlusCircle, Settings, Wallet, Calendar
} from 'lucide-react';
...
export const TransactionsPage = () => {
  ...
  const handleAiTextSubmit = async (e: FormEvent) => {
    ...
    if (parsed && parsed.amount > 0) {
      ...
      if (!error) {
        toast.success(`✅ Eklendi: ${parsed.merchant || parsed.category} - ${new Intl.NumberFormat('tr-TR').format(parsed.amount)} ₺`);
        setAiInput('');
            ...
            {cards.map(card => {
              ...
              return (
                <div
                  ...
                  className={`relative min-w-[200px] p-4 rounded-2xl cursor-pointer transition-all shrink-0 group ${
                ...
                >
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 rounded-lg bg-black/30 text-white/50 hover:text-red-400 hover:bg-black/50 transition-all opacity-0 group-hover:opacity-100"
                  >
                    <Trash2 size={12} />
                  </button>
